"""Build high-confidence ID mapping and prior-season Trackman summaries."""
...
def main():
    ...
    tr=pd.read_csv("data/train.csv",usecols=cols); tmkey=pd.read_csv("data/trackman_history.csv",usecols=tcols)
    ...
    allcols=A.columns.union(B.columns); A=A.reindex(columns=allcols,fill_value=0); B=B.reindex(columns=allcols,fill_value=0)
    ...
    best_j=sim.argmax(1); reverse_best=sim.argmax(0); ordered=np.sort(sim,axis=1)
    confidence=sim[np.arange(len(A)),best_j]; margin=ordered[:,-1]-ordered[:,-2]
    # Conservative one-to-one mapping: lower thresholds increased coverage but
    # degraded temporal validation, so retain only high-confidence pairs.
    ok=(reverse_best[best_j]==np.arange(len(A)))&(confidence>=.90)&(margin>=.03)
    mapping=pd.DataFrame({"pitcher_id":A.index[ok],"pitcher_trackman_id":B.index[best_j[ok]],
                          "mapping_similarity":confidence[ok],"mapping_margin":margin[ok]})
    mapping.to_csv(os.path.join(out,"pitcher_trackman_mapping.csv"),index=False)
    print("mapped",len(mapping),"train row coverage",tr.pitcher_id.isin(mapping.pitcher_id).mean())

    metrics=["rel_speed","spin_rate","induced_vert_break","horz_break","extension","rel_height","rel_side","zone_speed"]
    tm=pd.read_csv("data/trackman_history.csv",usecols=["pitcher_trackman_id","season","pitch_type_group",*metrics])
    rows=[]
    for rec in mapping.itertuples(index=False):
        ...
        for target_season in range(2019,2026):
            ...
            row={"pitcher_id":str(rec.pitcher_id),"season":target_season,
                 "tm_mapping_similarity":rec.mapping_similarity,"tm_n":len(hist)}
            for c in metrics:
                row[f"tm_{c}_mean"]=hist[c].mean(); row[f"tm_{c}_std"]=hist[c].std()
            ...
            for p in ["fastball","breaking","offspeed","other"]: row[f"tm_{p}_rate"]=rates.get(p,np.nan)
            ...
            rows.append(row)
    table=pd.DataFrame(rows); table.to_csv(os.path.join(out,"trackman_prior_features.csv"),index=False)
    print("saved",table.shape)
